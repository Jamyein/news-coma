...
class ResponseParser:
    ...
    @classmethod
    def parse_deep_analysis_response(
        cls,
        items: List[NewsItem],
        content: str,
        logger=None
        ...
        替代原 _parse_deep_analysis_response 方法
        ...
        try:
            ...
            for item_data in data_list:
                try:
                    ...
                    if 0 <= index < len(items) and index not in processed_indices:
                        item = items[index]
                        cls._apply_deep_analysis(item, item_data, logger)
                        results.append(item)
                        processed_indices.add(index)
            ...
            for i, item in enumerate(items):
                if i not in processed_indices:
                    ErrorHandler.apply_deep_analysis_defaults(item)
                    results.append(item)
            ...
            return ErrorHandler.apply_batch_deep_analysis_defaults(items)
    ...
    def _apply_deep_analysis(
        ...
        # 验证credibility_score范围
        try:
            credibility = float(item_data.get('credibility_score', 5.0))
        ...
        if not isinstance(key_arguments, list):
            key_arguments = [str(key_arguments)] if key_arguments else []
        
        # 构建深度分析结果
        item.deep_analysis = {
            "core_insight": item_data.get('core_insight', '') or '',
            "key_arguments": key_arguments,
            "impact_forecast": item_data.get('impact_forecast', '') or '',
            "sentiment": sentiment,
